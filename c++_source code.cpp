...
template <typename T> void sortDescending(vector<pair<string, T>> &entries) {
...
Buffered_File_Reader::Buffered_File_Reader(const string &path,
                                           size_t bufferSizeKB)
    : bufferSize(bufferSizeKB * 1024), leftover("") {
  if (bufferSizeKB < 256 || bufferSizeKB > 1024)
    throw out_of_range("Buffer size must be between 256 KB and 1024 KB. Got: " +
...
size_t Buffered_File_Reader::bufferSizeKB() const { return bufferSize / 1024; }
...
bool Tokenizer::isWordChar(char c) { return isalnum((unsigned char)c) != 0; }
...
void Indexer::build(const string &version, const string &filePath,
                    size_t bufferKB) {
  index[version].clear();
...
long long Indexer::getFrequency(const string &version, const string &word) {
  ...
  if (index[version].find(lowerWord) == index[version].end())
    return 0;

  return index[version][lowerWord];
}
...
DiffQuery::DiffQuery(const string &version1, const string &version2,
                     const string &word) {
  ...
  try {
    ...
    for (int i = 1; i < argc - 1; i++) {
      ...
      if (flag == "--file") {
        ...
        version1 = val;
        i++;
      } else if (flag == "--version1") {
        version1 = val;
        i++;
      } else if (flag == "--version2") {
        version2 = val;
        i++;
      } else if (flag == "--buffer") {
        bufferKB = stoul(val);
    ...
    if (queryType == "diff" && (file2.empty() || version2.empty()))
      throw invalid_argument(
          "--file2 and --version2 are required for diff query.");
